...
- **`tests/ml/`** (**9** modulos): generadores deterministas, contratos de columna congelados, anomalias y su auditor, agregados, manifiesto, semillas y seed del catalogo. Los **dos ultimos no hablan de silos sinteticos** y su trampa es otra: `test_contraste_temas.py` y `test_emision_temas.py` son la suite del sistema de diseno (`design/`). **Fijan el tema de omision byte a byte**, porque hay capturas ya entregadas que dependen de que no se mueva un valor, y por eso se corren **antes** de tocar `design/sistema.py`, no despues. Miden ademas contraste y separacion dicromatica por combinacion tema x modo: un token nuevo que no cumpla su propia regla las pone en rojo sin que ninguna prueba del frontend se entere.
...
El frontend no vive aqui: **53** specs en `frontend/test/` (vitest + happy-dom). **No existe todavia** suite de capa semantica ni de agente ADK: no escribas helpers anticipandolas. **No hay CI**: la puerta es local.
