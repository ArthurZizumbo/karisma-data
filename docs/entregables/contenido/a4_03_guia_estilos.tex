...
\begin{uxdestacado}
{\uxhead\fontsize{13.5}{16}\selectfont\color{uxnavy}Guía de estilos de Karisma Data}\par
\vspace{6pt}
{\color{uxamber}\rule{60pt}{2.2pt}}\par
\vspace{8pt}
La arquitectura que la Actividad 3 validó ---cuatro categorías, dieciséis ramas y nueve facetas transversales--- llegó a esta actividad como contrato de navegación, y esta guía es la otra mitad del contrato: la que fija cómo se ve y cómo se comporta cada pieza que ocupa esas rutas. Las diez secciones siguientes cubren identidad ---la del informe y la del portal---, tipografía, el sistema de tokens con sus dos temas y sus dos modos, componentes, iconografía, imágenes, retícula, movimiento, accesibilidad y lenguaje.
...
Tres tamaños y ninguno intermedio: \textbf{16, 20 y 24 px}, uno por densidad: 16 px en celda de tabla, 20 px en botón y en chip, 24 px en encabezado de tarjeta.

\begin{center}
  \muestraTamanosIcono
  \captionof{figure}{El mismo icono a los tres tamaños del sistema, medido en el navegador y no redibujado: 16, 20 y 24 px, alineados a la línea base}
\end{center}

El grosor \textbf{no se ajusta por tamaño}: la familia traza sus formas con dos unidades sobre un lienzo de 24, de modo que el icono de 24 px presenta un trazo de 2 px y el de 16 px uno de 1{,}33 px. Es un solo dibujo escalado, y se deja así a propósito: la corrección óptica que engruesa el trazo en el tamaño pequeño exigiría tres dibujos por icono y una tercera variable que mantener alineada. Alineación a la línea base del texto que acompañan, no al centro óptico de la caja.
...
El inventario se lee \textbf{por función y no por forma}: cada renglón dice qué hace el icono en el portal, cómo se dibuja y con qué nombre se pide. Son \textbf{\cifraIconosDeclarados{}} iconos en cinco grupos funcionales, y la columna de la derecha es además el rótulo accesible con el que se anuncian, de modo que la tabla fija a la vez la forma y la palabra.

\input{estilo/a4_iconos_declarados}

Junto a estos, la interfaz emplea los iconos de servicio que todo producto necesita ---cerrar, ordenar una columna, desplegar un detalle, avanzar o retroceder---. No se listan uno a uno porque no significan nada propio del dominio: pertenecen a la misma familia, se rigen por las mismas reglas de tamaño, grosor y área táctil, y su significado es el convencional.
...
Las imágenes de la serie de entregables viven en la carpeta \texttt{imagenes/} de los entregables y suman dieciséis archivos: los ocho retratos de las personas de la Actividad 1, las seis escenas de contexto de los escenarios de la Actividad 2 y los dos logotipos de la portada. Todas se insertan en el documento a través de macros de estilo, nunca con medidas escritas caso por caso.
